...
class Model(object):
    ...
    def cal_auc(self, score_list, logo_score_list, struct_score_list, score_map_list, test_y_list, logo_y_list, struct_y_list, test_mask_list):
        flatten_y_list = np.array(test_y_list).ravel().astype(np.uint8)
        ...
        flatten_mask_list = np.concatenate(test_mask_list).ravel().astype(np.uint8)
    ...
    def test(self):
        ...
        for idx, (x, y, mask, name) in enumerate(tqdm(self.testloader, ncols=80)):
            ...
            self.save_img([x,*feature_map_vis_list, dis_amap, dir_amap, amap, mask_cpu], os.path.join(self.vis_root, 'test', category), img_name.replace('png', 'tiff'))
    ...
    def save_img(self, img_list, save_root, idx_name):
        # img_size_dirct = {'juice_bottle':[800,1600],
        #                   'splicing_connectors':[1700, 850],
        #                   'screw_bag':[1600, 1100],
        #                   'pushpins':[1700, 1000],
        #                   'breakfast_box':[1600, 1280]}
        # img_size = img_size_dirct[self.class_name]
        os.makedirs(save_root, exist_ok=True)
        input_frame = denormalize(img_list[0].clone().squeeze(0).cpu().detach().numpy())
        cv2_input = np.array(input_frame, dtype=np.uint8)
        # plt.figure()
        ...
        # print(img_list[6])
        # temp = cv2.resize(img_list[6], (img_size[0], img_size[1]))
        # plt.imsave(os.path.join(save_root, idx_name), temp, cmap='gray')
        #
        # print(img_list[6].shape)
        # cv2.imwrite(os.path.join(save_root, idx_name), temp)
...
if __name__ == '__main__':
    ...
    for classname in MVTec_CLASS_NAMES:
        ...
        opt.referenc_img_file =  f'data/mvtec_loco_anomaly_detection/{opt.class_name}/train/good/000.png'
        save_name = opt.model_name+'_'+opt.backbone_name + '_k=4'
        opt.resume = fr'F:\LW\RB-VIT\result_logo/{save_name}/weight/{opt.class_name}'
        # if test_no_mask == True:
